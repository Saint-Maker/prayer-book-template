...
import {getPrayers, addPrayer, editPrayer, setPrayers, deleteAllPrayer} from '../redux/slice/prayerSlice';
...
function Prayer() {
  ...
  const {isOpen: isInvalidFileAlertOpen, onOpen: onInvalidFileAlertOpen, onClose: onInvalidFileAlertClose} = useDisclosure();
        ...
        drawerBtns={
          <>
            <Button
              ...
              w="full"
              leftIcon={<BsUpload/>}
              justifyContent="flex-start">Download Prayerbook
            </Button>
            <div>
              <input type="file" ref={fileInputRef} onChange={uploadPrayerbook} style={{display: 'none'}} />
              <Button
                w="full"
                onClick={prayers.data.length === 0 ? () => triggerUpload() : () => onUploadAlertOpen()}
                leftIcon={<BsDownload/>}
                justifyContent="flex-start">Upload Prayerbook
              </Button>
            </div>
            <Button
              ...
              colorScheme="red">Delete all prayer
            </Button>
          </>
        }>
        <VStack my="6">
